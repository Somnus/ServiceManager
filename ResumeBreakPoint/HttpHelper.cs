...
namespace Xly.LBC.ResumeBreakPoint
{
    ...
    public class HttpHelper
    {
        ...
        private void SetRequest(HttpItem item)
        {
            ...
            //范围请求
            if (item.Range != null && item.Range.Item1 != null)
            {
                if (item.Range.Item2 != null)
                    request.AddRange(Convert.ToInt64(item.Range.Item1), Convert.ToInt64(item.Range.Item2));
                else
                    request.AddRange(Convert.ToInt64(item.Range.Item1));
            }
            //设置Post数据
            SetPostData(item);
            //设置最大连接
